...
function RequestAccept({ item }) {
	...
	return (
		<TouchableOpacity
			...
			onPress={() => requestAccept(item.request_sender.username)}
		>
...
function RequestRow({ item }) {
	...
	return (
		<Cell>
			<Thumbnail
				url={item.request_sender.thumbnail}
				size={76}
			/>
			<View
				style={{
	...
	return (
		<View style={{ flex: 1 }}>
			<FlatList
				...
				keyExtractor={item => item.request_sender.username}
			/>
		</View>
	)
}
